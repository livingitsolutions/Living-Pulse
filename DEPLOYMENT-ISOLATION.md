...
The repository intentionally has no default root `netlify.toml`, because it would apply to both projects and override conflicting dashboard build fields. Growth instead selects the named repository-root `netlify.growth.toml`; Public remains isolated in its self-contained base.
...
- Package directory: unset
- Configuration: `netlify.growth.toml`
...
Growth uses the repository root and explicitly selects `netlify.growth.toml`. Public must use `deploy/public-site` as its Base directory and clear its old `deploy/public` Package directory so configuration and dependency installation occur entirely within the isolated subtree. Do not copy build, publish, or functions values into dashboard fields; the selected repository configuration owns them.
...
1. Keep the Growth project's Base and Package directories unset/root and select `netlify.growth.toml` from the repository root.
