...
import { hasEnoughOptions, WRITTEN_FEEDBACK_MAX_LENGTH } from './validation'
...
function Create() {
  ...
  const [followType, setFollowType] = useState<'multiple_choice' | 'written_feedback'>('multiple_choice')
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setError(''); if (!hasEnoughOptions(options)) return setError('Add at least two response options.'); if (hasFollowUp && !followQuestion.trim()) return setError('Add a follow-up question.'); if (hasFollowUp && followType === 'multiple_choice' && !hasEnoughOptions(followOptions)) return setError('Add at least two follow-up options.'); setSaving(true); try { const followUp: FollowUp | null = hasFollowUp ? followType === 'written_feedback' ? { type: 'written_feedback', question: followQuestion } : { type: 'multiple_choice', question: followQuestion, options: followOptions } : null; const pulse = await api.create({ businessName, idea, question, options, followUp, allowUpdates }, acquisition); navigate(publishedPath(pulse.id, pulse.creatorKey || '')) } catch (err) { setError(err instanceof Error ? err.message : 'Could not create Pulse') } finally { setSaving(false) } }
  return <main className="form-page"><header className="form-nav"><Logo /><span>New Pulse</span></header><div className="form-layout"><aside><p className="eyebrow">Make the decision smaller</p><h1>What do you need to know?</h1><p>Keep it quick. One clear idea gets a clearer signal.</p><div className="step-mark"><b>01</b><span>Write<br />the question</span></div></aside><form onSubmit={submit} className="create-form">
    ...
    <div className="switch-row"><div><b>Ask one follow-up</b><span>Multiple choice or written feedback</span></div><button type="button" role="switch" aria-checked={hasFollowUp} className={`switch ${hasFollowUp ? 'on' : ''}`} onClick={() => setHasFollowUp(!hasFollowUp)}><i /></button></div>
    {hasFollowUp && <div className="inset"><fieldset className="follow-type"><legend>Follow-up type</legend><label><input type="radio" name="follow-type" checked={followType === 'multiple_choice'} onChange={() => setFollowType('multiple_choice')} />Multiple choice</label><label><input type="radio" name="follow-type" checked={followType === 'written_feedback'} onChange={() => setFollowType('written_feedback')} />Written feedback</label></fieldset><label><span>{followType === 'written_feedback' ? 'Feedback question' : 'Follow-up question'}</span><input value={followQuestion} onChange={(e) => setFollowQuestion(e.target.value)} maxLength={300} placeholder={followType === 'written_feedback' ? 'What would make Sunday delivery more useful for you?' : undefined} required />{followType === 'written_feedback' && <small>Ask customers to explain what they want, why they chose an answer, or what would make the idea more useful.</small>}</label>{followType === 'multiple_choice' && <OptionEditor options={followOptions} setOptions={setFollowOptions} />}</div>}
...
function PublicPulse() {
  const { id = '' } = useParams(); const [pulse, setPulse] = useState<PublicPulse | null>(null); const [selected, setSelected] = useState(''); const [follow, setFollow] = useState(''); const [followText, setFollowText] = useState(''); const [stage, setStage] = useState<'primary' | 'follow' | 'email' | 'done'>('primary'); const [email, setEmail] = useState(''); const [error, setError] = useState('')
  ...
  function submitWritten() { if (pulse?.allowUpdates) setStage('email'); else void finish(selected, '', undefined, followText) }
  async function finish(optionId = selected, followUpOptionId = follow, submittedEmail?: string, submittedText = followText) { try { await api.respond(id, { optionId, followUpOptionId: followUpOptionId || undefined, followUpText: submittedText || undefined, email: submittedEmail || undefined }); setStage('done') } catch (e) { setError(e instanceof Error ? e.message : 'Could not save response') } }
  ...
  return <main className="public-shell"><div className="public-top"><Logo /><span>Quick question</span></div><section className="public-card">
    ...
    {stage === 'follow' && pulse.followUp && pulse.followUp.type !== 'written_feedback' && <><p className="step-count">One quick follow-up</p><h1>{pulse.followUp.question}</h1><div className="answer-grid">{pulse.followUp.options.map((option) => <button key={option.id} onClick={() => chooseFollow(option.id)}>{option.label}<ArrowRight size={20} /></button>)}</div></>}
    {stage === 'follow' && pulse.followUp?.type === 'written_feedback' && <><p className="step-count">Optional written feedback</p><h1>{pulse.followUp.question}</h1><form onSubmit={(e) => { e.preventDefault(); submitWritten() }}><label><span>Tell us what you think (optional)</span><textarea rows={5} value={followText} maxLength={WRITTEN_FEEDBACK_MAX_LENGTH} placeholder="Tell us what you think..." onChange={(e) => setFollowText(e.target.value)} /><small className="character-count">{followText.length}/{WRITTEN_FEEDBACK_MAX_LENGTH}</small></label><button className="button wide">Continue</button></form></>}
...
function ResultsPage() {
  ...
  return <main className="results-page"><header><Logo /><div className="results-actions"><button className="button secondary" onClick={() => void copyResultsLink()}>{copied ? <Check size={18} /> : <Clipboard size={18} />}{copied ? 'Copied' : 'Copy Results Link'}</button><ButtonLink to="/create" secondary>New Pulse</ButtonLink></div></header><section className="results-head"><div><p className="eyebrow">Private creator view · Declared customer interest</p><h1>{data.pulse.idea}</h1><p>{data.pulse.question}</p></div><div className="status-control"><p>Track what happens to this idea after testing.</p><label className="status-select"><span>Signal status</span><select value={data.pulse.status} onChange={(e) => void changeStatus(e.target.value)}>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select></label><p className="status-description" aria-live="polite">{statusDescriptions[data.pulse.status]}</p>{statusError && <p className="error" role="alert">{statusError}</p>}</div></section>
    ...
    {data.pulse.followUp?.type === 'written_feedback' && <section className="written-feedback-results"><p className="eyebrow">Customer feedback</p><h2>Customer feedback</h2>{data.writtenFeedback.length > 0 && <p>{data.writtenFeedback.length} written {data.writtenFeedback.length === 1 ? 'response' : 'responses'}</p>}<p className="feedback-question">“{data.pulse.followUp.question}”</p>{data.writtenFeedback.length > 0 ? <div className="feedback-list">{data.writtenFeedback.map((response, index) => <blockquote key={index}>{response}</blockquote>)}</div> : <p>No written feedback yet.</p>}</section>}
