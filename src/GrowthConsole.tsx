...
import { ArrowLeft, Check, ChevronRight, LogOut, Menu, ShieldCheck, X } from 'lucide-react'
import type { GrowthAttempt, GrowthOverview, GrowthProspectDetail, GrowthProspectSummary, GrowthSuppression } from './growthTypes'

type View = 'overview' | 'prospects' | 'outreach' | 'suppression'
type DialogAction = 'reject' | 'suppress' | 'queue'
const views: Array<{ id: View; label: string }> = [{ id: 'overview', label: 'Overview' }, { id: 'prospects', label: 'Prospects' }, { id: 'outreach', label: 'Outreach' }, { id: 'suppression', label: 'Suppression' }]
...
function Detail({ prospect, onBack, onMutate }: { prospect: GrowthProspectDetail; onBack(): void; onMutate(action: string, body?: object): Promise<void> }) {
  ...
  return <><button className="growth-back" onClick={onBack}><ArrowLeft size={17} />All prospects</button><div className="growth-detail-head"><div><p className="growth-kicker">Prospect record</p><h1>{prospect.businessName}</h1><p>{prospect.industry || 'Industry not recorded'} · {prospect.locationText || 'Location not recorded'}</p></div><div><Status value={prospect.qualificationStatus} kind="qualification" /><Status value={prospect.outreachStatus} kind="outreach" /></div></div>
    ...
    <div className="growth-detail-grid">
      ...
      <section><h2>Public contact</h2><dl><div><dt>Business email</dt><dd>{prospect.publicContactEmail}</dd></div><div><dt>Source type</dt><dd>{prospect.sourceType.replaceAll('_', ' ')}</dd></div><div><dt>Source URL</dt><dd><a href={prospect.sourceUrl} target="_blank" rel="noreferrer">Inspect source</a></dd></div><div><dt>Observed</dt><dd><DateText value={prospect.sourceObservedAt} /></dd></div></dl></section>
      <section><h2>Personalization</h2>{prospect.personalizationContext ? <dl><div><dt>Context</dt><dd>{prospect.personalizationContext}</dd></div><div><dt>Evidence</dt><dd>{prospect.personalizationEvidence ? <a href={prospect.personalizationEvidence} target="_blank" rel="noreferrer">Inspect evidence</a> : '—'}</dd></div></dl> : <p className="growth-muted">No personalization evidence recorded.<br />Future outreach will use generic copy.</p>}</section>
